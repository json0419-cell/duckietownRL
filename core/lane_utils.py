...
NON_DRIVABLE_TILE_KINDS = frozenset({"3way"})


def _tile_is_drivable(tile):
    if tile is None:
        return False
    if not tile.get("drivable", False):
        return False
    kind = tile.get("kind")
    return kind not in NON_DRIVABLE_TILE_KINDS
...
def tile_info(lp_cal, pos):
    ...
    return (i, j), tile.get("kind"), _tile_is_drivable(tile)
