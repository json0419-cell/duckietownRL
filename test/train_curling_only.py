...
DEFAULT_LOOP_MODEL = "./runs_db21j_loop_only_v2/ppo_db21j_seg19_loop.zip"


def parse_args():
    parser = argparse.ArgumentParser(description="Run Main.py in curling-only mode.")
    parser.add_argument("--timesteps", type=int, default=500_000, help="total training timesteps")
    parser.add_argument("--segment-timesteps", type=int, default=16_384, help="segment size")
    parser.add_argument("--max-episode-steps", type=int, default=500, help="TimeLimit wrapper")
    parser.add_argument(
        "--logdir",
        type=str,
        default="./runs_db21j_curling_only",
        help="output directory",
    )
    parser.add_argument(
        "--load-model",
        type=str,
        default=DEFAULT_LOOP_MODEL,
        help="resume from an existing PPO .zip",
    )
...
def main() -> int:
    ...
    print("[INFO] curling-only command:", " ".join(cmd))
